...
public partial class frmStaffschedulingNew : System.Web.UI.Page
{
    ...
    private void GVMainBind()
    {
        ...
        {
            if (Convert.ToString(Session["username"]) == "PMSAdmin" || Convert.ToString(Session["username"]) == "EGE7557")
            {
                gvStaffScheduling.Columns[9].Visible = true;
            }
            else
            {
                gvStaffScheduling.Columns[9].Visible = false;
            }
    ...
    protected void btnSaveNew_Click(object sender, EventArgs e)
    {
        ...
        if (dtentry != null)
        {
            if (dtentry.Rows.Count > 0)
            {
                for (int i = 0; i < dtentry.Rows.Count; i++)
                {
                    ...
                    string jjj = dtentry.Rows[i]["EntryDoneByName"] + "(" + dtentry.Rows[i]["ParticiparticipateName"] + ")";
                    UserName += "" + jjj + "" + ",";
        ...
        {
            ScriptManager.RegisterStartupScript(Page, GetType(), "Message", "<SCRIPT LANGUAGE='javascript'>alert('Please select Entry Done By')</script>", false);
        ...
        else
        {
            ...
            DataTable dtPhase = objComman.LoadData("select  isnull(N_P1_Y1,0) as [NoOfDays]  from Tbl_TB_Training Where  TrainingType='S' and OutComeID='" + ddlLearning.SelectedValue + "' and SoutComeID='" + ddlInducation.SelectedValue + "' and StateCode='" + ddlState.SelectedValue + "' and DistrictCode='" + ddlDistrict.SelectedValue + "'");
            ...
            if (dtPhase.Rows.Count > 0)
            {
                if (Convert.ToInt32(dtPhase.Rows[0]["NoOfDays"]) == 0)
                {
                    ...
                    SqlParameter[] parm = new SqlParameter[]
                        ...
                        new SqlParameter("@Inducation", ddlInducation.SelectedValue),
                    ...
                    int result = Convert.ToInt32(SqlHelper.ExecuteScaler(SqlHelper.mainConnectionString, CommandType.StoredProcedure, "InsertUpdateStaffScheduling2020", parm));
    ...
    public void AnnaualFCReportss(Int32 Flag)
    {
        ...
        {
            ExporttoExcel(dataTable, "StafftrainingSchedularReport");
